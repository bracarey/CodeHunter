...
//Removed Preprocessor "stdafx.h"
#include <iostream>
#include <string>
#include <stdlib.h> //Included "stdlib.h" to make the system("pause") work
...
int main()
{
	...
	//Deleted variable foo as it was not being used
	int vowels = 0;
	int consonants = 0;
	int digits = 0;
	int spaces = 0;
	...
	int checkSum = 0;
	//Deleted variable bar as it was not being used
	...
	for (int i = 0; i < textToAnalyze.length(); ++i)
	{
		if (textToAnalyze[i] == 'a' || textToAnalyze[i] == 'e' || textToAnalyze[i] == 'i' ||
			...
			textToAnalyze[i] == 'U')
		{
			++vowels; //Was set to decrease the value of vowels, Changed the -- to ++
		}
		else if ((textToAnalyze[i] >= 'a' && textToAnalyze[i] <= 'z') || (textToAnalyze[i] >= 'A' && textToAnalyze[i] <= 'Z'))
		{
			++consonants; //This line was made as a comment so I deleted the backslashes
		}
